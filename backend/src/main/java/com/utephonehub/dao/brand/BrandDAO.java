package com.utephonehub.dao.brand;

import com.utephonehub.model.brand.Brand;
...
public class BrandDAO {
    ...
    // Lấy brands theo category ID (cho API /brands?categoryId=1)
    public List<Brand> getBrandsByCategory(int categoryId) {
        List<Brand> list = new ArrayList<>();
        String sql = "SELECT id, name, category_id, status, created_at, updated_at FROM brands WHERE category_id=? AND status=true ORDER BY name ASC";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
