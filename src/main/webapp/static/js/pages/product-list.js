...
function renderCategories() {
  ...
  container.innerHTML = categories.map(cat => '<div class="filter-option" data-filter="category" data-value="' + cat.id + '">' +
...
function renderBrands() {
  ...
  container.innerHTML = brands.map(brand => '<div class="filter-option" data-filter="brand" data-value="' + brand.id + '">' +
      '<input type="checkbox" id="brand' + brand.id + '" ' +
      (currentFilters.brand == brand.id ? 'checked' : '') + '>' +
      '<label for="brand' + brand.id + '">' + escapeHtml(brand.name) + '</label>' +
    '</div>'
  ).join('');
}
    ...
    if (response && response.success && response.data) {
      ...
      if (products.length === 0) {
        ...
        if (metadata && metadata.pagination) {
          ...
          resultCount.textContent = 'Hiển thị ' + products.length + ' / ' + totalItems + ' sản phẩm';
...
function renderProducts(products) {
  ...
  productGrid.innerHTML = products.map(product => {
    ...
    html += '<img src="' + escapeHtml(product.thumbnailUrl || 'https://via.placeholder.com/300x200') + '" ';
...
function renderPagination(pagination) {
  ...
  // Previous button
  html += '<button class="pagination-btn" onclick="goToPage(' + (page - 1) + ')" ' + 
    (page === 1 ? 'disabled' : '') + '>';
...
function handleFilterChange(e) {
  ...
  if (filterType === 'category') {
    currentFilters.category = e.target.checked ? filterValue : '';
